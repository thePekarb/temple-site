...
export default function ContactsPage() {
  const [data, setData] = useState({ address: '', phone: '', info: '' });
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ address: '', phone: '', info: '' });
    ...
    <main className="min-h-screen pt-24 px-4 md:px-6 max-w-6xl mx-auto pb-16">
      ...
      <div className="grid md:grid-cols-2 gap-8 md:gap-12">
        ...
        <div className="space-y-6">
          ...
          <AnimatedSection delay={0.1}>
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-[#C5A059]/20 relative overflow-hidden">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Редактировать контакты">
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <button className="w-full bg-[#762121] text-white py-3 rounded-lg hover:bg-[#C5A059] transition font-medium">Сохранить</button>
        </form>
      </Modal>
    </main>
  );
